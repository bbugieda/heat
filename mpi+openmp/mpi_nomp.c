...
int	taskid,                     /* this task's unique id */
	...
	if (taskid != MASTER) {
		...
		MPI_Recv(&u[0][offsetx][offsety], 1, block_type, source, msgtype, mpi_comm_cart, MPI_STATUS_IGNORE);
...
/**************************************************************************
 *  subroutine test_endtests converges=all values changes by less than a threshold
 ****************************************************************************/
int test_end(int startx, int endx, int starty, int endy, int ny, float *u1, float *u2) {
	...
	for (ix = startx; ix <= endx; ix++) 
		for (iy = starty; iy <= endy; iy++) {
			if (fabs(*(u2+ix*ny+iy) - *(u1+ix*ny+iy)) >= EPS)
				return 0;
		}
	return 1;
}
