...
int	taskid,                     /* this task's unique id */
	...
	if (taskid == MASTER) {		
		...
		for (i=1; i<=dims[0]; i++) {
			/*handles cases so that no process gets only part of the border of the table-like the arr[x][0] column*/
	...
	for (it = 1; it <= STEPS; it++) {
		...
		MPI_Start(&recvd[iz]);
		/*main update*/
		update(startx+1,endx-1,starty+1,endy-1,NYPROB,&u[iz][0][0],&u[1-iz][0][0]);
		...
		/*test convergence*/
		if (it%PERIOD == 0) {
			endval = test_end(startx,endx,starty,endy,NYPROB,&u[iz][0][0],&u[1-iz][0][0]);
			/*reduce local convergence results*/
			MPI_Allreduce (&endval, &endres, 1, MPI_INT, MPI_LAND, mpi_comm_cart);

			if (endres) {
				if (taskid == MASTER)				
					printf ("Terminating at step %d\n", it);
				break;
			}
		}
...
void update(int startx, int endx, int starty, int endy, int ny, float *u1, float *u2) {
	int ix, iy;
	#pragma omp parallel for private(ix, iy) num_threads(4) collapse(2)
...
/**************************************************************************
 *  subroutine test_end-tests converges=all values changes by less than a threshold
 ****************************************************************************/
int test_end(int startx, int endx, int starty, int endy, int ny, float *u1, float *u2) {
	...
	int val = 1;
	int local_val;
	#pragma omp parallel for private(ix, iy, local_val) num_threads(4) reduction(&:val)
	for (ix = startx; ix <= endx; ix++) {
		local_val = 1;
		for (iy = starty; iy <= endy; iy++) {
